      ...
      <div className="relative overflow-hidden">
        ...
        <ul className="relative flex flex-col gap-y-6 md:gap-y-0">
          {educationAndExperience.map((item, index) => (
            <li
              key={index}
              className="group flex flex-col items-end justify-center
            ...
            >
              ...
              <TimelineDate
                ...
                className="right-4 mb-2 mr-4 flex items-center gap-x-2 self-end
                sm:absolute sm:mr-0 md:mb-0 md:self-center
                md:group-odd:left-1/2 md:group-odd:translate-x-10
                md:group-even:right-1/2 md:group-even:-translate-x-10"
              />
              <motion.div
                className="absolute right-3/4 mr-2 h-[4px] w-1/5 bg-amber-300
                sm:left-[48px] sm:mr-0 sm:w-[48px] md:left-[initial]
                md:w-[40px] md:group-odd:right-1/2 md:group-even:left-1/2
                lg:w-[48px] xl:w-[56px]"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              />
        ...
        <motion.div
          className="absolute left-[30px] top-0 h-full w-[4px] rounded-full
          bg-amber-300 md:left-1/2 md:-translate-x-1/2"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        />
        <TimelinePattern
          className="absolute bottom-0 left-[2px] rotate-180 md:left-[initial]
          md:right-1/2 md:translate-x-[10px]"
        />
      </div>
    </section>
  );
